...
async function fetchAllPlaylistVideos(playlistId: string): Promise<PlaylistInfo> {
  ...
  const initialResponse = await fetch(`https://www.youtube.com/playlist?list=${playlistId}`, {
    headers: {
      "User-Agent":
  ...
  const playlistContent =
    ytData?.contents?.twoColumnBrowseResultsRenderer?.tabs?.[0]?.tabRenderer?.content?.sectionListRenderer
  ...
  for (const item of playlistContent) {
    if (item?.playlistVideoRenderer) {
      ...
      if (video.isPlayable === false) continue

      const videoId = video.videoId
      const title = video.title?.runs?.[0]?.text || "Untitled"
      const durationText =
        video.lengthText?.simpleText || video.lengthText?.accessibility?.accessibilityData?.label || "0:00"
      ...
      const thumbnailUrl = thumbnails[thumbnails.length - 1]?.url || `https://i.ytimg.com/vi/${videoId}/mqdefault.jpg`
  ...
  let pageCount = 0
  const maxPages = 20 // Safety limit (20 pages * ~100 videos = 2000 videos max)

  while (continuationToken && pageCount < maxPages) {
    pageCount++

    console.log(`[v0] Fetching page ${pageCount + 1}, current video count: ${videos.length}`)
    ...
    try {
      ...
      if (!continueResponse.ok) {
        console.log(`[v0] Continuation request failed with status ${continueResponse.status}`)
        break
      }

      const continueData = await continueResponse.json()
      const continuationContents =
        continueData?.onResponseReceivedActions?.[0]?.appendContinuationItemsAction?.continuationItems || []
      ...
      for (const item of continuationContents) {
        if (item?.playlistVideoRenderer) {
          ...
          const thumbnails = video.thumbnail?.thumbnails || []
          const thumbnailUrl =
            thumbnails[thumbnails.length - 1]?.url || `https://i.ytimg.com/vi/${videoId}/mqdefault.jpg`
...
export async function POST(req: Request) {
  try {
    ...
    console.log(
      `[v0] Playlist "${playlist.title}" - ${playlist.videos.length} videos, ${Math.floor(playlist.totalDurationSeconds / 3600)}h ${Math.floor((playlist.totalDurationSeconds % 3600) / 60)}m`,
    )
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to fetch playlist information" },
      { status: 500 },
    )
  }
}
